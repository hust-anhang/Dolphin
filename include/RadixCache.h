...
struct CacheNodeValue {
  ...
  volatile void * next; // the pointer to child node
...
private:
  ...
  //RadixCache
  //concurrent_queue<CacheNode*>* node_queue        |     cache_entry_gc      |      cache_hdr_gc      |     eviction_list
  //CacheHeader   +            CacheMap             |        CacheEntry       |       CacheHeader
  //depth+partial |  map<uint8_t, CacheNodeValue>   |
  //              |            CacheEntry + *next   |
  //              |depth+addr+vector<InternalEntry> |
};
