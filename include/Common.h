...
#define MAX_KEY_SPACE_SIZE 1024 * define::MB
...
// app thread
#define MAX_APP_THREAD 35    // one additional thread for data statistics(main thread)  [config]
...
// Remote Allocation
constexpr uint64_t dsmSize           = 5;        // GB  [CONFIG]，32
...
// Cache (MB)
constexpr int kIndexCacheSize = 200;
...
// Leaf Node
constexpr uint32_t kLeafPageSize = 1088; // newly added
constexpr int64_t SMOstatus = -1000000; // newly added 
constexpr int64_t SMOUpLimit = -100000; // newly added
constexpr int64_t normalOp = 1;         // newly added
...
constexpr uint64_t kKeyMax = MAX_KEY_SPACE_SIZE;  // only for int workloads
#endif
