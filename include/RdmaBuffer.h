...
private:
  ...
  static const int kFaaBufferCnt    = 256;  // big enough to hold batch internal node write in out_of_place_write_node
  ...
  uint64_t *cas_buffer;
  uint64_t *faa_buffer; // newly added
  char *page_buffer;
  char *leaf_buffer;
  ...
  int cas_buffer_cur;
  int faa_buffer_cur;    // newly added
  int page_buffer_cur;
  int leaf_buffer_cur;
...
public:
  ...
  void set_buffer(char *buffer) {
    ...
    header_buffer = (uint64_t *)((char *)leaf_buffer   + define::kLeafPageSize * kLeafBufferCnt);
  ...
  uint64_t *get_faa_buffer() {  // newly added
  ...
  char *get_leaf_buffer() {
    ...
    return leaf_buffer + leaf_buffer_cur * define::kLeafPageSize;
  }
