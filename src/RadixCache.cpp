...
bool RadixCache::search_from_cache(const Key& k, volatile CacheEntry**& entry_ptr_ptr, CacheEntry*& entry_ptr, int& entry_idx) {
  ...
  if(_search(byte_array, ret)) {
    while(!ret.empty()) { // Checking from bottom to top layer by layer, so it is efficient
      const auto& item = ret.top();
      auto cache_entry = item.entry_ptr;
      auto next_partial = k.at(item.next_idx);
      if (cache_entry) {
        if (item.next_idx != define::keyLen - 1) {
          ...
          for (int i = 0; i < (int)cache_entry->records.size(); ++ i) {
            ...
            if (target == 0 && e.partial == target) { // prefix=0
              pos = i;
              find = true;
            } else if (e.partial <= next_partial && e.partial > target) {
              target = e.partial;
              pos = i;
              find = true;
              // printf("next_partial: %d, target: %d, pos: %d\n", next_partial, target, pos);
            }
          }
          ...
          if (find) {
            ...
            entry_ptr_ptr = item.entry_ptr_ptr;
            entry_idx = pos;
            // printf("here 2, pos: %d, record.size: %d, cache_entry->records[pos]= %d, partial: %d\n", pos, cache_entry->records.size(), cache_entry->records[pos], cache_entry->records[pos].partial);
            return true;
          }
        }
      }
...
// From SMART, replace it with the following function
void RadixCache::search_range_from_cache(const Key &from, const Key &to, std::vector<RangeCache> &result) {
  ...
  for (int i = 0; i < 3; i++) { // simulating three leaf nodes that consist of 100 data items in total
    if (e) {
      if(entry_ptr->records[entry_idx+i].val != 0) {
        int idx = entry_idx+i;
        assert(idx >= 0);
        if (idx >= 8)
          break;

        p_ptr = GADD(entry_ptr->addr, sizeof(InternalEntry) * idx);
        p = entry_ptr->records[idx];
        depth = entry_ptr->depth;
      } else if (entry_ptr->records[entry_idx-i].val != 0) {
        int idx = entry_idx-i;
        if (idx < 0)
          break;
...
//   int depth;
//   int entry_idx = -1;
//   for(Key k = from; k < to; k = k + 1){
//     CacheKey byte_array(k.begin(), k.begin() + define::keyLen - 1);
//     SearchRetStk ret; // result
//     if(_search(byte_array, ret)){
//       while(!ret.empty()){
//         auto& item = ret.top();
//         auto cache_entry = item.entry_ptr;
//         auto next_partial = k.at(item.next_idx);
//         if(cache_entry){
//           depth = cache_entry->depth;
//           if(depth == define::keyLen){
//             Key nextfrom = get_leftmost(k, define::keyLen - 1);
//             if(nextfrom < from){
//               nextfrom = from;
//             }
//             Key nextto = get_rightmost(k, define::keyLen - 1);
//             if(nextto > to - 1){
//               nextto = to - 1;
//             }
//             uint8_t from_partial = nextfrom.at(define::keyLen - 1);
//             uint8_t to_partial = nextto.at(define::keyLen - 1);
//             for (int i = 0; i < (int)cache_entry->records.size(); ++ i){
//               const auto& e = cache_entry->records[i];
//               if (e != InternalEntry::Null() && e.partial >= from_partial && e.partial <= to_partial) {
//                 entry_ptr = cache_entry;
//                 entry_ptr_ptr = item.entry_ptr_ptr;
//                 entry_idx = i;
//                 p_ptr = GADD(entry_ptr->addr, sizeof(InternalEntry) * entry_idx);
//                 p = entry_ptr->records[entry_idx];
//                 auto leftmost = remake_prefix(k, define::keyLen, e.partial);
//                 auto rightmost = remake_prefix(k, define::keyLen, e.partial);
//                 result.push_back(RangeCache(leftmost, rightmost, p_ptr, p, depth, entry_ptr_ptr, entry_ptr));
//               }
//             }
//             k = nextto;
...
//                 p = entry_ptr->records[entry_idx];
//                 depth = entry_ptr->depth;
//                 auto leftmost = get_leftmost(k, depth);
//                 auto rightmost = get_rightmost(k, depth);
//                 result.push_back(RangeCache(leftmost, rightmost, p_ptr, p, depth, entry_ptr_ptr, entry_ptr));
//                 find = true;
//                 k = rightmost;
//                 break;
