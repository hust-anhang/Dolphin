...
void DSM::faa(GlobalAddress gaddr, int64_t add_val, uint64_t *rdma_buffer, bool signal, CoroContext *ctx) {

  if (ctx == nullptr) {
    rdmaFetchAndAdd(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer, remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset, add_val,
                     iCon->cacheLKey, remoteInfo[gaddr.nodeID].dsmRKey[0], signal);
  } else {
    rdmaFetchAndAdd(iCon->data[0][gaddr.nodeID], (uint64_t)rdma_buffer, remoteInfo[gaddr.nodeID].dsmBase + gaddr.offset, add_val,
...
void DSM::faa_sync(GlobalAddress gaddr, int64_t add_val, uint64_t *rdma_buffer, CoroContext *ctx) {
